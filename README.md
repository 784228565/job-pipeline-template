...
**🚀 首选：让 AI Agent 替你装。** 如果你使用 Kimi Code 或 WorkBuddy，不必手动执行任何命令——让 agent 读取本仓库的引导文件 <https://github.com/784228565/job-pipeline-template/blob/master/docs/agent_bootstrap_prompts.md>，按其中的引导 prompt 执行即可。
...
```
 ...
 06:00  ┌──────────┐   ┌───▼────┐   ┌───────────┐
───────▶│ 1 搜索    │──▶│ 岗位池 │──▶│ 2 筛选     │──▶ 当日队列 xlsx ──────────┐
        │ (Web)    │   │ xlsx   │   │ 去重/闸门  │                              │
        └──────────┘   └────────┘   │ 评分排序   │                              ▼
                                    └───────────┘   12:30 ┌─────────┐    ┌──────────┐
                                                           │ 3 预检   │──▶│ 4 投递    │──▶ delivery_*.csv
                                                           │(CDP只读) │    │ (CDP写)  │
                                                           └────┬────┘    └────┬─────┘
                                     platform_login_status.json ─┘            │
 15:00  ┌──────────┐                                                          │
 ──────▶│ 5 通知   │◀── applicable_today.csv + delivery_status.csv + delivery_log.csv
        │ (邮件)   │
        └──────────┘
```
